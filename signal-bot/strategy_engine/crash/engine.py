...
from strategy_engine.crash.multiplier import MultiplierStrategy
from strategy_engine.crash.timing import TimingStrategy
...
class CrashEngine:
    ...
    async def evaluate(self, result: dict, result_id: int):
        ...
        for strategy_name, open_sig in list(self._open_signals.items()):
            ...
            if status == "loss" and gale < open_sig.get("max_gale", 1):
                new_gale = gale + 1
                await update_signal_status(sig_id, "loss")
                new_id = await dispatch_signal("crash", result["table_id"], strategy_name, entry, new_gale, result_id)
                self._open_signals[strategy_name] = {**open_sig, "gale": new_gale, "signal_id": new_id}
                logger.info(f"[Crash/{strategy_name}] GALE {new_gale}: {entry}")
            else:
                await update_signal_status(sig_id, status)
                del self._open_signals[strategy_name]
                logger.info(f"[Crash/{strategy_name}] Resolvido: {status.upper()}")
        ...
        for strategy in strategies:
            ...
            if should:
                sig_id = await dispatch_signal(
                    "crash", result["table_id"], strategy.name, entry, 0, result_id
                )
                ...
                logger.info(f"[Crash/{strategy.name}] ENTRADA: {entry}")
